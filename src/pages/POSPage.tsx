    ...
    try {
      ...
      if (!useMockData) {
        try {
          ...
          let response = await fetch(`${API_BASE}/produits-stock/scan`, {
            method: 'POST',
            headers: {
      ...
      const mockProduct = MOCK_PRODUCTS[qrCode as keyof typeof MOCK_PRODUCTS];
      ...
      // Vérifier que le produit n'est pas déjà dans le panier
      const dejaEnPanier = panier.find(item => item.id === mockProduct.id);
      if (dejaEnPanier) {
        throw new Error('Ce produit est déjà dans le panier');
      }
    ...
    try {
      if (!useMockData) {
        ...
        logActivity({
          type: 'sale',
          module: 'POS',
          description: `Vente démo effectuée - Total: €${calculerTotal().toFixed(2)}`,
          metadata: {
        ...
        showToast({
          ...
          message: `Total: €${calculerTotal().toFixed(2)} - Mode démo`
        });
      }
        ...
        >
          ...
          <Card className="p-6 hover:shadow-lg transition-all duration-300">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-nexsaas-vanta-black dark:text-gray-300 mb-4">
                Utilisez la caméra pour scanner les codes QR des produits
              </p>
              <Button 
                onClick={() => setShowScanner(true)}
                disabled={isLoading}
                className="w-full bg-nexsaas-saas-green hover:bg-green-600"
              >
          ...
          <Card className="p-6 hover:shadow-lg transition-all duration-300">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-nexsaas-vanta-black dark:text-gray-300 mb-4">
                Entrez manuellement le code QR ou le SKU du produit
              </p>
              <Button 
            ...
            {panier.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-nexsaas-vanta-black dark:text-gray-400 mb-6">
                  Scannez ou ajoutez des produits pour commencer une vente
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-4 text-sm text-nexsaas-vanta-black dark:text-gray-300">
                            ...
                            <span>Lot: {produit.lot}</span>
                            <span>QR: {produit.qrCode}</span>
                          </div>
                          <p className="text-xs text-nexsaas-vanta-black dark:text-gray-400">
                            Ajouté le {produit.dateAjout.toLocaleTimeString()}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-600">
                  ...
                  >
                    ...
                    Procéder au paiement (€{calculerTotal().toFixed(2)})
                  </Button>
                </div>
              </div>
            )}
            ...
            <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
              <div className="flex items-start">
                ...
                <div className="text-sm text-yellow-700 dark:text-yellow-300">
                  ...
                  {useMockData && (
                    <p className="mt-2 font-bold">⚠️ Mode démo: Les données ne seront pas enregistrées en base.</p>
                  )}
                </div>
              </div>
            </div>
