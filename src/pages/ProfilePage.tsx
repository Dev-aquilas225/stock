...
import { useProfile, ExtendedUpdateProfileDto } from '../hooks/useProfile';

const ProfilePage: React.FC = () => {
    const { user, loading, handleUpdateProfile, handleUploadSelfie, handleUpdatePassword } = useProfile();
    const { showToast } = useToast();
    const { logActivity } = useActivity();
    const navigate = useNavigate();
    const [imageKey, setImageKey] = useState(0); // For cache-busting
    const [isImageModalOpen, setIsImageModalOpen] = useState(false); // For large image view
    ...
    // Profile form state aligned with ExtendedUpdateProfileDto
    const [profileForm, setProfileForm] = useState<ExtendedUpdateProfileDto>({
        nom: user?.nom || '',
        prenom: user?.prenom || '',
        phone: user?.phone || '',
    ...
    const validateProfileForm = () => {
        ...
        if (!profileForm.nom) newErrors.nom = 'Prénom requis';
        if (!profileForm.prenom) newErrors.prenom = 'Nom requis';
    ...
    const handleProfileSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.message === "Non autorisé : Veuillez vous reconnecter") {
                showToast({
                    type: 'error',
        ...
        try {
            ...
            if (err.message === "Non autorisé : Veuillez vous reconnecter") {
                ...
                showToast({
                    type: 'error',
                    title: 'Erreur',
                    message: err.message || 'Une erreur est survenue lors de la modification',
                    duration: 4000,
                });
            }
        }
    ...
    const handleImageError = async (e: React.SyntheticEvent<HTMLImageElement>) => {
        console.error("Image load error:", user?.profilePicture, e);
        try {
    ...
    const updateProfileForm = (field: keyof ExtendedUpdateProfileDto, value: string) => {
    ...
    return (
        <div className="min-h-screen pt-16 bg-gradient-to-br from-nexsaas-pure-white to-nexsaas-light-gray dark:from-nexsaas-vanta-black dark:to-gray-900">
            <div className="container mx-auto px-4 py-8 max-w-4xl">
                    ...
                    <Card>
                        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
                            <div className="relative">
                                <div className="w-24 h-24 bg-nexsaas-deep-blue rounded-full flex items-center justify-center overflow-hidden cursor-pointer" onClick={() => user?.profilePicture && !imageError && setIsImageModalOpen(true)}>
                                    {user?.profilePicture && !imageError ? (
                                        <img
                                            src={`${user.profilePicture}?t=${imageKey}`}
                                            alt="Profile"
                                ...
                                <label
                                    className="absolute -bottom-2 -right-2 w-8 h-8 bg-nexsaas-saas-green rounded-full flex items-center justify-center text-white hover:bg-green-600 transition-colors cursor-pointer"
                                >
                            ...
                            <div className="flex-1 text-center md:text-left">
                                ...
                                <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                                    ...
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400">
                                        ...
                                        {user?.actif ? "Compte vérifié" : "Compte non vérifié"}
                                    </span>
                ...
                {/* Image Modal */}
                {isImageModalOpen && user?.profilePicture && !imageError && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="relative bg-nexsaas-pure-white dark:bg-gray-800 p-4 rounded-lg max-w-3xl w-full">
                            <button
                                className="absolute top-2 right-2 text-gray-600 dark:text-gray-300 hover:text-nexsaas-deep-blue dark:hover:text-nexsaas-pure-white"
                            ...
                            <form onSubmit={handleProfileSubmit} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Input
                                        ...
                                        onChange={(value) => updateProfileForm("nom", value)}
                                        icon={User}
                                        error={errors.nom}
                                        required
                                    />
                                    <Input
                                        ...
                                        onChange={(value) => updateProfileForm("prenom", value)}
                                        icon={User}
                                        error={errors.prenom}
                                        required
                                    />
                                ...
                                {user?.type === "entreprise" && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <Input
                                            ...
                                            onChange={(value) => updateProfileForm("companyName", value)}
                                            icon={Building}
                                        />
                                        <Input
                                            label="NIF"
                                            value={profileForm.nif}
                                            onChange={(value) => updateProfileForm("nif", value)}
                            ...
                            <form onSubmit={handlePasswordSubmit} className="space-y-6">
                                ...
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                                    ...
                                    <ul className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
                                        <li className={`flex items-center ${passwordForm.newPassword.length >= 6 ? "text-green-600" : ""}`}>
                                            <CheckCircle className={`w-3 h-3 mr-2 ${passwordForm.newPassword.length >= 6 ? "text-green-500" : "text-gray-400"}`} />
                                            Au moins 6 caractères
                                        </li>
                                        <li className={`flex items-center ${/[A-Z]/.test(passwordForm.newPassword) ? "text-green-600" : ""}`}>
                                            <CheckCircle className={`w-3 h-3 mr-2 ${/[A-Z]/.test(passwordForm.newPassword) ? "text-green-500" : "text-gray-400"}`} />
                                            Une lettre majuscule (recommandé)
                                        </li>
                                        <li className={`flex items-center ${/[0-9]/.test(passwordForm.newPassword) ? "text-green-600" : ""}`}>
                                            <CheckCircle className={`w-3 h-3 mr-2 ${/[0-9]/.test(passwordForm.newPassword) ? "text-green-500" : "text-gray-400"}`} />
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-nexsaas-light-gray dark:bg-gray-700 rounded-lg">
                                        <div className="flex items-center">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-nexsaas-vanta-black dark:text-gray-300">
                                                    Aujourd'hui à 14:30 • Paris, France
                                                </p>
                                            </div>
                                        </div>
                                    </div>
