...
export interface ProductPrixVenteDto {
    prixVente?: string;
}
...
export const addProduct = async (
    ...
    return {
        ...
        prixVente: parseFloat(response.data.prixVente) || 0,
        ...
        priceHistory: response.data.priceHistory
            ? response.data.priceHistory.map((ph: any) => ({
                  id: ph.id,
                  price: parseFloat(ph.price) || 0,
                  date: ph.date,
                  negotiatedBy: ph.negotiatedBy,
...
export const updateProduct = async (
    ...
    const response = await axiosClient.patch(
        `fournisseurs/produits/${productId}`,
        data,
        {
            headers: {
                Authorization: `Bearer ${token}`,
...
export const updateProductPrixVente = async (
    productId: string,
    data: ProductPrixVenteDto,
): Promise<Product> => {
    const token = localStorage.getItem("token");
    const response = await axiosClient.patch(
        `fournisseurs/produits/${productId}/prixVente`,
        data,
        {
            headers: {
                Authorization: `Bearer ${token}`,
